...
#load control data
ctr_data <- read.table("/work/IdaHeleneDencker#2808/Exam/Data/Own_data/own.txt",header=TRUE)
# The no starting amount group
...
print('fitting model on 1 (no start group)')
...
print(samples)
#Save 
capture.output(print(samples), file = "no_start_jags_model_report_1_part.txt")


# Plot traceplots for all the parameters
png("no_start_traceplots_parameters_1_part.png", 
    width = 1000, 
    height = 1000,  
    res = 150)
...
ggsave('/work/IdaHeleneDencker#2808/Exam/Code/Parameter_estimation/no_start_posteriors_parameters_1_part.png', plot = one_part, width = 12, height = 8, dpi = 300)
...
print('fitting model on all (no start group)')
samples <- jags.parallel(data, inits=NULL, params,
...
#save
save(samples,
     file="no_start_JAGS_ORL_hier.RData")
print('saving samples')

#loading in data 
#load("JAGS_ORL_hier_no_start.RData")

#look at model diagnostics and convergence
print(samples)
#Save 
capture.output(print(samples), file = "no_start_jags_model_report_all_part.txt")
#dev.off()


# Plot traceplots for all the parameters
png("no_start_traceplots_parameters_all_part.png", 
    width = 1000, 
    height = 1000,  
    res = 150)
...
ggsave('/work/IdaHeleneDencker#2808/Exam/Code/Parameter_estimation/no_start_posteriors_parameters_all_part.png', plot = all_part, width = 12, height = 8, dpi = 300)

print('done running parameter estimation no start')